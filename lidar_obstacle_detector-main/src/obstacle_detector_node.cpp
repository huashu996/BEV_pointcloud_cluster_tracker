...
void dynamicParamCallback(lidar_obstacle_detector::obstacle_detector_Config& config, uint32_t level)
{
  ...
  //ROI_MAX_POINT = Eigen::Vector4f(100, 100, 100, 1);
  //ROI_MIN_POINT = Eigen::Vector4f(-100, -100, -100, 1);
...
ObstacleDetectorNode::ObstacleDetectorNode() : tf2_listener(tf2_buffer)
{
  ...
  pub_jsk_bboxes = nh.advertise<jsk_recognition_msgs::BoundingBoxArray>(jsk_bboxes_topic, 1);  //发布boundingbox
...
void ObstacleDetectorNode::lidarPointsCallback(const sensor_msgs::PointCloud2::ConstPtr& lidar_points)
{
  ...
  pcl::fromROSMsg(*lidar_points, *raw_cloud);
	...
	outrem.setInputCloud(new_cloud);    //设置输入点云
	outrem.setRadiusSearch(0.8f);    //设置半径为0.8的范围内找临近点
	...
	pcl::transformPointCloud(*cloud_filtered, *trans_cloud, transform);//用pcl不要用pcl_ros
  // Downsampleing, ROI, and removing the car roof
  auto filtered_cloud = obstacle_detector->filterCloud(trans_cloud, VOXEL_GRID_SIZE, ROI_MIN_POINT, ROI_MAX_POINT);
  ...
  // Publish Obstacles 发布聚类点云 和 boundingbox
...
void ObstacleDetectorNode::publishDetectedObjects(std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr>&& cloud_clusters, const std_msgs::Header& header)
{
  for (auto& cluster : cloud_clusters)
  {
    ...
    Box box = USE_PCA_BOX? 
      obstacle_detector->pcaBoundingBox(cluster, obstacle_id_) :  //pca包围盒子
...
//jsk_recognition_msgs/BoundingBoxArray是一种ROS消息类型，用于表示包含一组3D边界框的数组。每个3D边界框由其最小和最大三维坐标表示。移动操作std::move()可以将jsk_bboxes的所有权转移到另一个变量中，这在避免不必要的数据拷贝时非常有用。
